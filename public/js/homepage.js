...
function updateChart() {
        ...
        if (dataset.label == `${text.CONFIRMED} ${localCountry.name}`) {
            ...
            setTimeout(
                setBackground.bind(
                    ...
                    datasets[i + 1].backgroundColor,
                    datasets[i + 2].backgroundColor
                ),
                1500
            );
    ...
    $(".no-nations").remove();
    $("#section1").append(
        `<div><p class="delete-country" onclick="countryCheckbox(this)" id="${localCountry.code}"><img src="/img/cross.svg" alt="Delete" style="max-width: 1rem;"> <img src="/img/flags/${localCountry.code}.png" alt="Flag" style="max-width: 1rem;"> <span class="country-name strike-link">${localCountry.name}</span></p></div>`
    );
...
function getCountry(callback) {
    ...
    $.ajax({
        ...
        error: function (xhr, status, error) {
            alert(
                `${xhr.status} ${xhr.statusText}, ${text.CANT_REQUEST} /${localCountry.name}: ${xhr.responseText}. ${text.ERROR_LETMEKNOW}`
            );
        },
        complete: function () {
...
function updateCountry() {
    localCountry = {
        name: $("#country_selector")
            ...
            .name.replace(/ *\([^)]*\) */g, ""),
        code: $("#country_selector").countrySelect("getSelectedCountryData")
            .iso2
    };
}
    ...
    globalChart.data.datasets.forEach(function (dataset, index) {
        if (
            ...
            if ($("#section1").text().trim() == text.SELECTED_COUNTRIES + ":") {
                $("#section1").append(
                    `<p class="no-nations text-muted" style="font-size: 0.8rem">${text.NO_COUNTRIES}</p>`
                );
            }
            globalChart.update();
            return false;
...
const shareInterval = setInterval(function () {
